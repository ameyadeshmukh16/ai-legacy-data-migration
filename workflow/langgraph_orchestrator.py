...
def human_review_gate(s):
    ...
    by={int(x["mapping_index"]):x for x in decisions}; approved=[]
    for i,x in enumerate(m):
        if i not in by: raise PermissionError("Missing human decision")
...
if __name__=="__main__":
    ...
    result=app.invoke({"run_id":rid,"status":"started"},config={"configurable":{"thread_id":rid}})
